...
# styles = ['r-', 'g-', 'y-', 'm-', 'k-', 'c-'] # x2?
lines = [ax.plot(x, y, 'r-')[0] for ax in axes.flat]
# titles = [ax.set_title(mNames[0])[0] for ax in axes.flat]
for ax in axes.flat:
    ax.set_title(mNames[0] + mValues[0])
    ax.set_xlabel("t[s]")
...
def save2LOG():
    ...
    for x in DailyDataMerged: # DailyDataMerged
        logger.info(x)
#         logger.info('\n')
    ...
    if (timer_flag + 1) % T1 == 0: # Sprawdzic przypadek, gdy krotnosc T1 pokrywa sie z T2
        ...
        for h in range(11):
            if len(TempLists[h]) > 0:
                ...
                # Dodaj wartosc do wykresu
                for j, line in enumerate(lines, start=1):
                    line.set_ydata(val)
                fig.canvas.draw()    
#                 fig.canvas.flush_events()
                plt.scatter(timer,val, 1)
