...
def stoper():
    ...
    while True:
        ...
        for x in range(T2): # Mozna zmienic na dokladniejsze liczenie czasu poprzez roznice czasu systemowego
            # Okresowy zapis wartosci srednich z czasie T1 do przebiegu dziennego
            if (timer + 1) % T1 == 0:
                for h in range(11):
                    if len(TempLists[h]) > 0:
                        DailyData[h].append(sum(TempLists[h])/len(TempLists[h]))
                        TempLists[h] *= 0
...
#             print(timer)
        ...
        # Zgraj dane za okres T1 do danych dziennych
