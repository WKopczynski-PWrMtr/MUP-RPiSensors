...
# Parametry czasowe
T1 = czasZapisuTxtT1 # 60-600s
T2 = 60 * czasZapisuTxtT2 # 10-120min -> 600-7200s
T3 = 60 * czasZapisuTxtT3 # 1-1440min -> 60-86400s
timer = 0
noDataStartTime = 0

# 
TempLists = [termAT1, termBT1, termCT1, ADC1T1, RTCT1, F1T1, F2T1, WDST1, CPRT1, SHARP30T1, SR04td]
...
#
    # Termometr B
        ...
        pomiar = hex2int(Data,8) * 1 # Do podmienienia przelicznik
...
#             print(bitCnt)
    ...
    # Pozostale elementy ramki
        ...
        if A > 16: # Jesli nadal poza zakresem -> nie sa HEXem
            checksum = -1 # Ramka nieprawidlowa
            print(str(Addr) + " | Ramka nieprawidlowa - znak nie nalezy do HEX")
...
#                 print(LenD)
            ...
            # ComL
                ...
                ComL = FByte
                if hex2int(ComL) != (0 - hex2int(LenD)): # Sprawdzenie liczby kontrolnej (ComL = 00h - LenD)
                    checksum = -1
...
def USARTbuffer(): # Buforowanie danych (Watek 2)
    global strBuffer, strBuffer2, stopRec, noDataStartTime
    cnt = 0
    while True:
...
def stoper(): # Stoper + aktywacje flag dla okresow T1, T2, T3 (Watek 3)
    global timer, T2, T1, T3, strBuffer2, noDataStartTime
    ...
    while True:
        for x in range(T2):
            ...
            if (math.floor(perf_counter() - noDataStartTime) % 10 == 0 and len(ilosc danych gotowych w RAM) > 0) or timer % T3 == 0: # Aktywowanie flagi w czasie T3 lub w przypadku braku danych do odczytu w czasie 10s
                flagSave2SRV = 1
